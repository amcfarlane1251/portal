...
function groups_handle_copy_page($guid = 0) {
	gatekeeper();
	$group = get_entity($guid);
	...
	if ($group && $group->canEdit()) {
		elgg_set_page_owner_guid($group->getGUID());
		elgg_push_breadcrumb($group->name, $group->getURL());
		$content = elgg_view("groups/copy", array('entity' => $group));
	} else {
		$content = elgg_echo('groups:noaccess');
	}
	...
	$params = array(
		'content' => $content,
		'title' => elgg_echo('groups:copy'),
		'filter' => '',
	);
	$body = elgg_view_layout('content', $params);

	echo elgg_view_page(elgg_echo('groups:copy'), $body);
}

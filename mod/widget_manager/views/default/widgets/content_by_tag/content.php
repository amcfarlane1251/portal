<?php 
	...
	$displayContentTypes=array(elgg_echo("widget_manager:widgets:content_by_tag:discussions") => "thewire", elgg_echo("widget_manager:widgets:content_by_tag:blogs") => "blog", elgg_echo("widget_manager:widgets:content_by_tag:files") => "file", elgg_echo("widget_manager:widgets:content_by_tag:questions") => "question", elgg_echo("widget_manager:widgets:content_by_tag:pages") => "page", elgg_echo("widget_manager:widgets:content_by_tag:bookmarks") => "bookmarks");
	
	foreach($displayContentTypes as $heading => $content_type){								
	
		if(isset($_GET['more'])){
			if($_GET['more']!=$content_type){
				continue;
			} else {
				$count=20;
			}
		...
		*/
		...
		if($content_type == "page"){
			...
			$content_type = array("page","page_top");
		} else {
		...
		*/
		...
		/* INSERT CODE HERE TO DECIDE TAGS BASED ON URL THAT THIS WIDGET IS USED IN */
		//this session var is set in the corresponding start.php
		$role="norole";
		$roleTags="";		
		if(isset($_SESSION['role'])){
			switch($_SESSION['role']){
				case "learner":
					$tags="Learner-Apprenant, learner, learners, student, trainee, apprenant, apprenants, éDtudiant, stagiaire";
					
					//$tags="Learner-Apprenant";
					break;
				case "instructor":
					$tags="Instructor-Instructeur, instructor, instructors, teacher, professor, instructeur, instructeurs, professeur";
					//$tags="Instructor-Instructeur";
					break;
				case "developer":
					$tags="Developer-Développeur, developer, developers, déloppateur, déloppeurs";
					//$tags="Developer-Développeur";
					break;
				case "trainingmgr":
					$tags="Training-Manager, trainingmgr";
					//$tags="Training Manager-Gestionnaire de la formatio";
					break;
			}			
			$roleTags=$tags;
			$role=$_SESSION['role'];
		...
		$values = string_to_tag_array($roleTags);
		...
		$options = array(
				"type" => "object",
				//commented out so that ALL content types will be returned (elgg_get_entities has a series of defaults when $options are omitted)
		...
		if(empty($result)){
			...
			if(isset($_GET['more'])){
				if($_GET['more']!=$content_type){
					echo "<div class=\"roleTagHeading $role\"><h3>".$heading."</h3><a href=\"./\">".elgg_echo("widget_manager:widgets:content_by_tag:viewLess")."</a></div>";
				}
			} else {
				echo "<div class=\"roleTagHeading $role\"><h3>".$heading."</h3><a href=\"./?more=".$content_type[0]."\">".elgg_echo("widget_manager:widgets:content_by_tag:viewMore")."</a></div>";
			}
			echo $result;
		}

					...
					if (!empty($user)) {
						if (!$group->isMember($user)) {
							if (!check_entity_relationship($group->getGUID(), "invited", $user->getGUID()) || $resend) {
								...
								*/
								$url = elgg_get_site_url()."groups/invitations/" .$user->__get('username');

								$group_name = $group->get('name');
								$user_name = $user->__get('name');

								$subject = "Group invitation from " . $group_name;
								$body = "Hi ".$user_name.",<br /><br />";
								$body .= "You have been invited to join the group " . $group_name . ".";
								$body .= $text;
								$body .= "<br /><a href='".$url."'>Click here</a> to view your active group invitations.";
								...
								*/
								...
								if (group_tools_invite_user($group, $user, $text, $resend)) {
									...
									//send notification to users inbox
									notify_user($user_guid, $from, $subject, $body, array(), 'site');
								}
							} else {

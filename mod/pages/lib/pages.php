...
function pages_check_if_checked_out($page){
	$checkee = get_entity($page->checkedOut)->name;
	register_error(elgg_echo('pages:checked_out_by', array($checkee)));
	forward($page->getURL());
}

function check_out_page($page, $userGuid){
	$page->checkedOut = $userGuid;
	if($page->save()){
		return true;
	}
}

function check_in_page($page, $userGuid){

;(function (tinymce, $) {
    ...
    AutoComplete.prototype = {
        ...
        renderInput: function() {
            var rawHtml =  '<span class="autocomplete">' +
                                '<span class="autocomplete-delimiter">' + this.options.delimiter + '</span>' +
                                '<span class="autocomplete-searchtext"><span class="dummy">\uFEFF</span></span>' +
                            '</span>';
        ...
        bindEvents: function() {
            this.editor.onKeyUp.addToTop(this.editorKeyUpProxy = $.proxy(this.rteKeyUp, this));
            this.editor.onKeyDown.addToTop(this.editorKeyDownProxy = $.proxy(this.rteKeyDown, this));
            this.editor.onClick.addToTop(this.editorClickProxy = $.proxy(this.rteClicked, this));
            $(this.editor.getWin()).live('scroll', this.rteScroll = $.proxy(function(){ this.cleanUp(true); }, this));
            $(document).delegate('.rte-autocomplete', 'click', $.proxy(this.autoCompleteClick, this));
            //$('body').live('click', this.bodyClickProxy = $.proxy(this.rteLostFocus, this));
        },

        unbindEvents: function() {
            this.editor.onKeyUp.remove(this.editorKeyUpProxy);
            this.editor.onKeyDown.remove(this.editorKeyDownProxy);
            this.editor.onClick.remove(this.editorClickProxy);
            $(document).undelegate('.rte-autocomplete', 'click', $.proxy(this.autoCompleteClick, this));
        },

        rteKeyUp: function(ed, e) {
        ...
        show: function() {
            var rtePosition = $(this.editor.getContainer()).offset(),
                ...
                top = rtePosition.top + toolbarPosition.innerHeight() + nodePosition.top + $(this.editor.selection.getNode()).innerHeight() - $(this.editor.getDoc()).scrollTop() + 40,
